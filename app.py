...
elif option == "Load existing metadata":
            ...
            if metadata_path:
                try:
                    with st.spinner("Loading Metadata..."):
                        # Passes the manual path to utils.py
...
elif option == "Test application":
    with st.expander("Test Environment", expanded=True):
        st.info("Click below to instantly load the data and Run Search Engine.")
        
        if st.button("Load MetaData", type="primary"):
            try:
                with st.spinner("Initializing demo environment..."):
                    # Call load_metadata with NO arguments! It will use the default secure path.
                    metadata = load_metadata() 
                    st.session_state.metadata = metadata
                    st.session_state.unique_classes, st.session_state.count_options = get_unique_classes_counts(metadata)
                    st.success("Demo loaded successfully! Scroll down to search.")
            except Exception as e:
                st.error(f"Deployment Error: Ensure data/processed/metadata.json exists. Details: {str(e)}")
